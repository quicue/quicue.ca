...
i006: core.#Insight & {
	...
	evidence: [
		...
		"CMHC retrofit, maison-613, homelab all consume the same patterns and produce domain-specific projections",
...
i009: core.#Insight & {
	...
	evidence: [
		"E2E deployment on fresh Ubuntu 24.04 VM (VM 201 on node-3, 198.51.100.32) hit all 8 traps",
...
i014: core.#Insight & {
	...
	evidence: [
		"homelab has custom K8s/Proxmox schemas with hostname/ip/role fields -- no @type or depends_on",
	...
	action_items: [
		...
		"Add homelab as a worked example of incremental adoption",
	]
