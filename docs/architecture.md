...
`.kb/downstream.cue` registers known consumers (homelab, cjlq, maison-613, apercue). Each consumer maintains its own `.kb/` with a deps registry cataloging which vocab and pattern definitions it imports and where they're used — for example, homelab's deps.cue records 14 pattern and 2 vocab definitions with source, purpose, and consuming files.
